...
pub use maa_value_macro::object;
...
impl MAAValue {
    ...
    ///
    /// let mut obj = MAAValue::default();
    /// obj.maybe_insert("present", Some("value"));
    /// obj.maybe_insert("absent", None::<&str>);
    ///
    ...
    /// - [`Self::try_insert`] for fallible conversions.
    ...
    /// Inserts a key-value pair into the object with fallible conversion.
    ///
    /// Unlike [`Self::insert`], this method accepts any value that implements
    /// `TryInto<MAAValue>`. This is useful for types like `PathBuf`, `OsString`, or `&Path`
    /// that may fail to convert (e.g., if they contain invalid UTF-8).
    ///
    /// If the key already exists, the value will be replaced.
    ///
    /// # Errors
    ///
    /// Returns an error if the value conversion fails.
    ///
    /// # Panics
    ///
    /// Panics if `self` is not an object variant.
    ...
    /// obj.try_insert("path", path).unwrap();
    ///
    /// assert_eq!(obj.get("path").unwrap().as_str().unwrap(), "/some/path");
    /// ```
    ///
    /// # See also
    ///
    /// - [`Self::insert`] for infallible conversions
    /// - [`Self::maybe_insert`] for optional values
    pub fn try_insert<V>(&mut self, key: impl Into<String>, value: V) -> Result<()>
    where
        V: TryInto<Self>,
        error::Error: From<<V as TryInto<MAAValue>>::Error>,
    {
        if let Self::Object(map) = self {
